...
def draw_paragraph(vg, x, y, width, height, text, font_size = 44.0, name = 'sans')
  ...
  lineh = lineh_buf.unpack1('F')

  text_start = text
  text_end = nil
  while (nrows = NVG.TextBreakLines(vg, text_start, text_end, width, rows_buf, 3)) != 0
    rows = nrows.times.collect do |i|
      NVG::TextRow.new(rows_buf + i * NVG::TextRow.size)
    end
    nrows.times do |i|
      row = rows[i]
      # pp [nrows, i, vg, x, y, row[:start], row[:end], row[:next], row[:width], row[:min], row[:max]]

      NVG.BeginPath(vg)
      NVG.FillColor(vg, WHITE)
...
if __FILE__ == $0
  ...
  while GLFW.WindowShouldClose(window) == 0
    ...
    winWidth = winWidth_buf.unpack1('L')
    winHeight = winHeight_buf.unpack1('L')
    fbWidth = fbWidth_buf.unpack1('L')
    fbHeight = fbHeight_buf.unpack1('L')
